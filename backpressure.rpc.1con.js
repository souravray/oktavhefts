const { Client, ConsumerGroupStream, KeyedMessage, ProducerStream } = require('kafka-node'),
  ...
  upstreamTopic = 'back.pressure.up',
  downstreamTopic = 'back.pressure.down.1';
...
const consumerOptions = {
  ...
  groupId: 'OktavheftBackpressureRpctGroup',
...
/*      CONSUMER - MID     */
/* ========================*/
const midstreamConsumer = new ConsumerGroupStream(consumerOptions, upstreamTopic);
  ...
  transform (message, encoding, callback) {
    ...
    if( mesObj.method==='one' ) {
      setTimeout(() =>  {
        ...
        midstreamConsumer.commit(message, true);
        callback(null, {
